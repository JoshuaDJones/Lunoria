...
const DashboardDialogList = ({
    sceneDialogs
}: DashboardDialogListProps) => {
    const modalRouter = useModalRouter();

  return (
    <div className='flex flex-col flex-1 m-2 gap-2'>
        {sceneDialogs.map(d => 
        <button className="rounded bg-stone-600 flex p-3 hover:opacity-50" onClick={() => modalRouter.push(<ViewSceneDialogModal sceneDialog={d} />)}>
            <Text className="flex-1 text-start" textColor={TextColor.white}>{d.title}</Text>
