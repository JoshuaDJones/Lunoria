    ...
    : IPlaythroughRepository
{
        ...
        CancellationToken ct)
    {
        return dbContext.ScenePTs
            ...
            .Include(scene => scene.SceneCharacters)
                .ThenInclude(sceneCharacter => sceneCharacter.Spells)
            .Include(scene => scene.SceneCharacters)
                .ThenInclude(sceneCharacter => sceneCharacter.EquippableItems)
                    .ThenInclude(link => link.PlaythroughEquippableItem)
            .Include(scene => scene.SceneEvents)
                .ThenInclude(sceneEvent => sceneEvent.ScenePTActionEvents)
                    .ThenInclude(action => action.CharacterStatAdjustmentAction)
            .Include(scene => scene.SceneEvents)
                .ThenInclude(sceneEvent => sceneEvent.ScenePTActionEvents)
                    .ThenInclude(action => action.CharacterAddSpellAction)
                        .ThenInclude(grant => grant!.PlaythroughSpell)
            .Include(scene => scene.SceneEvents)
                .ThenInclude(sceneEvent => sceneEvent.ScenePTActionEvents)
                    .ThenInclude(action => action.CharacterGiveItemAction)
                        .ThenInclude(grant => grant!.PlaythroughConsumableItem)
            .Include(scene => scene.SceneEvents)
                .ThenInclude(sceneEvent => sceneEvent.ScenePTActionEvents)
                    .ThenInclude(action => action.CharacterGiveItemAction)
                        .ThenInclude(grant => grant!.PlaythroughEquippableItem)
            .Include(scene => scene.SceneEvents)
                .ThenInclude(sceneEvent => sceneEvent.ScenePTActionEvents)
            ...
            .Include(scene => scene.Playthrough)
                .ThenInclude(playthrough => playthrough.JourneyCharacters)
                    .ThenInclude(journeyCharacter => journeyCharacter.Spells)
            .Include(scene => scene.Playthrough)
                .ThenInclude(playthrough => playthrough.JourneyCharacters)
                    .ThenInclude(journeyCharacter => journeyCharacter.EquippableItems)
                        .ThenInclude(link => link.PlaythroughEquippableItem)
            .Include(scene => scene.Playthrough)
                .ThenInclude(playthrough => playthrough.Scenes)
            .Include(scene => scene.Playthrough)
                .ThenInclude(playthrough => playthrough.JourneyCharacters)
    ...
    public Task<ScenePT?> GetSceneForEndAsync(
        int userId,
        int playthroughId,
        int sceneId,
        CancellationToken ct)
        ...
        CancellationToken ct)
    {
        return dbContext.ScenePTs
            ...
            .Include(scene => scene.Playthrough)
                .ThenInclude(playthrough => playthrough.ConsumableItems)
            .Include(scene => scene.Playthrough)
                .ThenInclude(playthrough => playthrough.EquippableItems)
                    .ThenInclude(item => item.AffectedSpellType)
            .Include(scene => scene.Playthrough)
                .ThenInclude(playthrough => playthrough.EquippableItems)
                    .ThenInclude(item => item.AddedSpells)
