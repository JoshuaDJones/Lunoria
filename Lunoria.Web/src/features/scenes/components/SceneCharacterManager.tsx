...
export function SceneCharacterManager({ scene }: Props) {
  ...
  const load = async () => {
    ...
    setError("");
    try {
      setAssignments(await listSceneCharacters(scene.id));
    } catch (requestError) {
  ...
  const openAttach = async () => {
    try {
      if (catalog.length === 0)
        setCatalog(
          await listCharacters({ typeFilter: CharacterType.Any, take: 500 }),
        );
      setView("attach");
    } catch (requestError) {
      toast.error(
  ...
  const openSpells = async (assignment: SceneCharacter) => {
    try {
      ...
      toast.error(getApiError(requestError).message, "Unable to load spells");
    }
  };
  ...
  if (isLoading)
    return (
      <p className="text-content-secondary">Loading scene characters...</p>
    );
  if (error)
    return (
    ...
    return (
      <EditForm
        ...
        onSave={async (input) => {
          ...
          setView("characters");
          toast.success(`${saved.character.name} was updated.`);
        }}
      />
    );
  }
  ...
  if (view === "spells" && selected) {
    return (
      <SpellManager
        ...
        onSave={async (spellIds) => {
          const saved = await replaceSceneCharacterSpells(
            selected.id,
            spellIds,
          );
          setAssignments((current) =>
            ...
            >
              <div className="flex items-center gap-3">
                {assignment.character.photoUrl ? (
                  ...
                  <span className="flex size-12 items-center justify-center rounded-lg bg-utility/15 text-utility-hover">
              ...
              <div className="mt-3 flex flex-wrap justify-end gap-2 border-t border-border pt-3">
                ...
                <Button
                  onClick={() => void openSpells(assignment)}
                  variant="magic"
                  size="sm"
...
function AttachForm({
  ...
  return (
    <form onSubmit={(event) => void submit(event)} className="space-y-5">
      <h3 className="text-2xl font-semibold text-content">Attach character</h3>
      {characters.length === 0 ? (
        <p className="text-content-secondary">
          No NPCs or enemies are available.
        </p>
      ) : (
        <FormField htmlFor="scene-character" label="NPC or enemy">
          <Select
            id="scene-character"
            value={id}
      ...
      <FormActions
        ...
        error={error}
        onCancel={onCancel}
        disableSave={characters.length === 0}
      />
    </form>
  );
}
...
function EditForm({
  ...
  const [values, setValues] = useState({
    ...
    bow: optionalNumber(assignment.bowAttackDamage),
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      await onSave({
        meleeAttackDamage: nullableNumber(values.melee),
        bowAttackDamage: nullableNumber(values.bow),
        ...
        alternateFormId: nullableNumber(values.alternate),
      });
    } catch (requestError) {
      setError(getApiError(requestError).message);
      setSaving(false);
    ...
    <form onSubmit={(event) => void submit(event)} className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <NumberField
          ...
          setValue={(v) => set("movement", v)}
        />
        <NumberField
          id="sc-consumables"
          label="Consumable slots"
          value={values.consumables}
          setValue={(v) => set("consumables", v)}
        />
        <NumberField
          id="sc-equipment"
          label="Equipment slots"
          value={values.equipment}
          setValue={(v) => set("equipment", v)}
        />
      </div>
      <FormField htmlFor="sc-alternate" label="Alternate form">
        <Select
          id="sc-alternate"
          value={values.alternate}
          onChange={(e) => set("alternate", e.target.value)}
        >
...
function SpellManager({
  ...
  const [selection, setSelection] = useState(
    () => new Set(assignment.sceneCharacterSpells.map((item) => item.spell.id)),
  );
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  ...
  return (
    <div>
      ...
      <h3 className="mt-5 text-2xl font-semibold text-content">
        {assignment.character.name} spells
      </h3>
      {spells.length === 0 ? (
        <p className="mt-4 text-content-secondary">No spells are available.</p>
      ) : (
        <div className="mt-5 space-y-2">
          {spells.map((spell) => (
            <label
              key={spell.id}
              className="flex cursor-pointer items-center gap-3 rounded-xl border border-border bg-surface p-4"
            >
              <input
                type="checkbox"
              ...
              <span>
                ...
                <span className="text-sm text-content-secondary">
                  {spell.spellType?.name ?? "Spell"} · MP {spell.mpCost}
                </span>
              </span>
            </label>
          ))}
  ...
  return (
    <div className="mt-5 rounded-xl border border-border bg-surface/60 p-8 text-center">
      <h3 className="text-xl font-semibold text-content">
        No scene characters yet
      </h3>
      <p className="mt-2 text-sm text-content-secondary">
        Attach the first character to this scene.
      </p>
    </div>
  );
}
